...
import { subscribeToGame } from "src/store2/subscriptions"
...
import * as net from "src/store2/network"
...
async function updateGameData(ID: bigint, player: Address): Promise<FetchedGameData> {
  ...
  console.groupCollapsed(`fetched game data (at ${formatTimestamp(timestamp)})`)
  console.dir(gameData)
  console.groupEnd()

  // TODO: only update if gameData actually changed — probably needs to check contract to make sure every interaction updates something
  // (e.g. lastBlockNum)
...
async function fetchCards(ID: bigint, player: Address): Promise<readonly bigint[]> {
  ...
  const timestamp = Date.now()
  console.log(`fetched cards (at ${formatTimestamp(timestamp)})`)
...
async function updateRandomness(ID: bigint, player: Address, gameData: FetchedGameData) {
  ...
  const timestamp = Date.now()
  console.log(`fetched randomness (at ${formatTimestamp(timestamp)})`)
...
export async function refreshGameData({ forceFetchCards = false } = {}) {
  ...
  if (!shouldFetchCards) {
    ...
    if (shouldUpdateCards()) {
      // Don't reassign if cards were fetched in the meantime.
