...
export const Modal = ({ ctrl, children }: { ctrl: ModalController; children: ReactNode }) => {
    ...
    return <>{loaded && createPortal(<ModalInner ctrl={ctrl}>{children}</ModalInner>, document.body)}</>
}
...
const ModalInner = ({ ctrl, children }: { ctrl: ModalController; children: ReactNode }) => {
    ...
    const dialogEscapeHandler = (event: Event) => {
        if (!ctrl.state.closeable)
            // state from useState might be stale!
    ...
    if (!state.loaded) console.error("Modal rendered but its loaded property is false")
...
export class ModalController {
     ...
     */
    set setLoaded(setLoaded: (_: boolean) => void) {
        this.setLoaded_ = setLoaded
    }
     ...
     * Only for use in the modal implementation!
     */
    set setState(setState: (_: ModalState) => void) {
    ...
    readonly close = () => {
        ...
        if (!this.state_.loaded) return // already in target state
        if (this.state_.closingHides) this.updateState({ displayed: false })
        else this.updateState({ loaded: false, displayed: false })
    }
    ...
    set closeableAndSurroundCloseable(closeable: boolean) {
        if (this.state_.closeable === closeable && this.state_.surroundCloseable === closeable) return // already in target state
