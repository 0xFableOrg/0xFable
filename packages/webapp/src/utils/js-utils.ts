...
export function bigintToHexString(n: bigint, extendTo?: number): string {
    ...
    return extendTo !== undefined ? str.padStart(extendTo * 2, "0") : str.length % 2 === 1 ? "0" + str : str
}
...
export function parseBigInt(
    ...
    if (value instanceof Uint8Array || Array.isArray(value)) {
        if (endianness == "little") value = value.reverse()
        return BigInt("0x" + bytesToHexString(value))
    }

    if (endianness == "little") throw new Error("little-endianness only supported with Uint8Array or number[] argument")
 ...
 */
export function parseBigIntOrNull(value: string | number | bigint | Uint8Array | number[] | null): bigint | null {
...
export function formatTimestamp(timestamp: number) {
    ...
    return [date.getHours(), date.getMinutes(), date.getSeconds()].map((num) => (num < 10 ? "0" + num : num)).join(":")
}
...
export const convertBigIntArrayToStringArray = (cardsList: readonly bigint[] | null): string[] => {
