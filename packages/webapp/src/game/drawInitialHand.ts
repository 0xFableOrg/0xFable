...
export function drawInitialHand(
    initialDeck: readonly bigint[],
    deckStartIndex: number,
    salt: bigint,
    ...
    for (let i = 0; i < initialDeck.length; i++) deckIndexes[i] = deckStartIndex + i
    for (let i = initialDeck.length; i < deckIndexes.length; i++) deckIndexes[i] = 255
    ...
    for (let i = 0; i * FELT_SIZE < MAX_DECK_SIZE; i++)
        deckRootInputs.push(parseBigInt(deckIndexes.slice(i * FELT_SIZE, (i + 1) * FELT_SIZE), "little"))
    for (let i = 0; i * FELT_SIZE < MAX_HAND_SIZE; i++)
        handRootInputs.push(parseBigInt(handIndexes.slice(i * FELT_SIZE, (i + 1) * FELT_SIZE), "little"))
