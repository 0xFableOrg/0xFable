...
export function getGameStatus(gdata: FetchedGameData | null, player: Address | null): GameStatus {
    if (gdata === null || player === null) return GameStatus.UNKNOWN
    ...
    if (gdata.currentStep === GameStep.UNINITIALIZED)
        if (!gdata.players.includes(player)) return GameStatus.CREATED
        else if (gdata.livePlayers.includes(gdata.players.indexOf(player))) return GameStatus.HAND_DRAWN
        else return GameStatus.JOINED

    if (gdata.currentStep === GameStep.ENDED) return GameStatus.ENDED
    else return GameStatus.STARTED
}
...
export function getOpponentAddress(gdata: FetchedGameData | null, player: Address | null): Address | null {
    if (gdata == null || player == null) return null
    if (gdata.players.length !== 2) throw new Error("Wrong assumption: game doesn't have exactly 2 players.")
...
export function getPrivateInfo(
    gameID: bigint | null,
    player: Address | null,
    privateInfoStore: PrivateInfoStore
): PrivateInfo | null {
    // Directly return null if either gameID or player is null, or if the specific info does not exist in the store.
    return gameID !== null && player !== null ? privateInfoStore[gameID.toString()]?.[player] || null : null
}
...
export function getPlayerHand(
    ...
    return handIndexes.slice(0, handSize).map((index) => gdata.cards[index])
}
 ...
 */
export function getDeck(pdata: PlayerData | null, cards: readonly bigint[] | null): bigint[] | null {
 ...
 */
export function getBattlefield(pdata: PlayerData | null, cards: readonly bigint[] | null): readonly bigint[] | null {
    if (pdata === null || cards === null) return null
    const battlefield = []
    for (let i = pdata.deckStart; i < pdata.deckEnd; i++)
        if (pdata.battlefield & (1n << BigInt(i))) battlefield.push(cards[i])
    return battlefield
}
