...
export function updatePlayerAddress(result: AccountResult) {
    ...
    const newAddress = result.status === "disconnected" || !isNetworkValid() ? null : result.address || null // undefined --> null
...
export function updateNetwork(result: NetworkResult) {
    if (result.chain === undefined) console.log("disconnected from network")
    else console.log(`network changed to chain with ID ${result.chain?.id}`)

    if (!isNetworkValid(result)) store.set(store.gameID, null) // resets all game data
...
export async function refreshGameData() {
    ...
    if (gameData.publicRandomness === 0n && status !== GameStatus.ENDED) {
        ...
        if (blockNum < block.number - 256n) {
            ...
            if (status === GameStatus.STARTED) {
                if (currentPlayer === player) {
                    ...
                    setError({
                        ...
                        message:
                            "Your opponent didn't take an action within the time limit, " + "and you won as a result.",
                ...
                if (status === GameStatus.HAND_DRAWN) {
                    ...
                    setError({
                        ...
                        message:
                            "Some players didn't join within the time limit, " + "the game got cancelled as a result.",
                        buttons: [{ text: "Leave Game", onClick: sendTimeout }],
                    })
                } else {
                    setError({
                        title: "Time out",
                        message:
                            "You couldn't join the game within the time limit, " +
            ...
            const lastGameBlock =
                block.number === gameData.lastBlockNum
                    ? block
    ...
    if (gameData.cards.length > 0) store.set(store.gameData, gameData)
    else store.set(store.gameData, { ...gameData, cards: store.get(store.cards) as any })

    const timestamp = Date.now()
    console.groupCollapsed(
        "updated game data " + (shouldFetchCards ? "(including cards) " : "") + `(at ${formatTimestamp(timestamp)})`
    )
 ...
 */
export async function waitForUpdate(blockNum: bigint, timeout: number = 15): Promise<FetchedGameData | null> {
    return new Promise<FetchedGameData | null>((resolve, _reject) => {
        ...
        const unsub = store.store.sub(store.gameData, () => {
            ...
            if (gameData === null || gameData.lastBlockNum >= blockNum) unsubAndResolve(gameData)
        })

        // Maybe the game data is already up to date.
        const gameData = store.get(store.gameData)
        if (gameData !== null && gameData.lastBlockNum >= blockNum) return unsubAndResolve(gameData)
