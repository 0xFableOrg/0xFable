...
async function playCardImpl(args: PlayGameArgs): Promise<boolean> {
    ...
    if (getCurrentPlayerAddress(gameData) !== playerAddress || gameData.currentStep !== GameStep.PLAY) return false // old/stale call
    ...
    checkFresh(
        await freshWrap(
            contractWriteThrowing({
                ...
                args: [gameID, newHandRoot, args.cardIndexInHand, card, proof.proof_a, proof.proof_b, proof.proof_c],
