...
async function drawCardImpl(args: DrawCardArgs): Promise<boolean> {
    ...
    if (getCurrentPlayerAddress(gameData) !== playerAddress || gameData.currentStep !== GameStep.DRAW) return false // old/stale call
    ...
    if (newHandLastIndex < 0) throw new Error("Hand is full") // TODO handle this more fundamentally
    ...
    const initialHandSize = tmpHandSize < 0 ? BigInt(privateInfo.handIndexes.length) : BigInt(tmpHandSize)
    ...
    checkFresh(
        await freshWrap(
            contractWriteThrowing({
                ...
                args: [gameID, newHandRoot, newDeckRoot, proof.proof_a, proof.proof_b, proof.proof_c],
...
if (typeof window !== "undefined") (window as any).drawCard = drawCard
